...
class PolyModel(unittest.TestCase):
    ...
    def test_save(self):
        ...
        self.failUnless(
            obj.content_type == ContentType.objects.get_for_model(PolyLeafModel)
        )

        # leaf class class name should be set on save.
        self.failUnless(obj.class_name == PolyLeafModel.__name__)

        # correct leaf class content type should be retained over base
        # class' content type.
        base = obj.polytrunkmodel_ptr
        base.save()

        self.failUnless(
            base.content_type == ContentType.objects.get_for_model(PolyLeafModel)
        )

        # correct leaf class class name should be retained over base
        # class' class name.
    ...
    def test_as_leaf_class(self):
        ...
        # always return the leaf class, no matter where we are in the
        # hierarchy.
        self.failUnless(PolyTrunkModel.objects.get(
            id=obj.id).as_leaf_class() == obj)
        self.failUnless(PolyBranchModel.objects.get(
            id=obj.id).as_leaf_class() == obj)
        self.failUnless(PolyLeafModel.objects.get(
            id=obj.id).as_leaf_class() == obj)
        ...
        self.failUnless(PolyTrunkModel.objects.get(
            id=trunk_obj.id).as_leaf_class() == trunk_obj)
...
class TestCase1378(unittest.TestCase):
    ...
    def test_resolve_to_name(self):
        ...
        self.assertEqual(
            resolve_to_name('/some/other/url/'),
            'this_is_a_named_view',
            'Return view name on named view match',
        )

        # Bogus URL should result in Resolver404.
        self.assertRaises(Resolver404, resolve_to_name, '/some/bogus/url/')


class TestCase2536(unittest.TestCase):

    def test_create_default_site(self):
        self.assertEqual(
            Site.objects.get().domain,
            'localhost:8000',
            'Should only have one site with domain set to localhost:8000.',
        )
